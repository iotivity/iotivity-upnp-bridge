...
OCEntityHandlerResult UpnpAvTransport::processGetRequest(string uri, OCRepPayload *payload, string resourceType)
{
    ...
    playStateValue = strstr(currentTransportStateValue, playAction) ||
            strstr(currentTransportStateValue, "Play") || strstr(currentTransportStateValue, "PLAY");
    if (!OCRepPayloadSetPropBool(payload, playStatePropertyName, playStateValue))
    {
        throw "Failed to set playState value in payload";
    }
    ...
    {
        throw "Failed to set mediaSpeed value in payload";
    }
    ...
    {
        throw "Failed to set mediaLocation value in payload";
    }
    ...
    if (!actions.empty())
    {
        ...
        {
            throw "Failed to set actions value in payload";
        }
    }
    ...
    {
        throw "Failed to set lastAction value in payload";
    }
    ...
    if (UPNP_OIC_TYPE_MEDIA_CONTROL == resourceType)
    {
        ...
        if (playStateValueIsPresent || mediaSpeedValueIsPresent || mediaLocationValueIsPresent || lastActionValueIsPresent)
        {
            ...
            if (lastActionValueIsPresent)
            {
                ...
                {
                    // Unknown action
                    string message = "Unknown action " + string(lastActionValue);
                    throw message;
                }
            }
            ...
            if (mediaSpeedValueIsPresent)
            {
                ...
                {
                    upnpPlaySpeedAsFract = lround(mediaSpeedValue * 10.0);
                    doUpnpPlay = true;
                }
                else
                {
        ...
        {
            throw "Payload must have one of playState, mediaSpeed, mediaLocation, or action";
        }
    }
    else
    {
