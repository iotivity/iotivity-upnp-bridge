...
#ifndef BINARYSWITCHMENU_H_
#define BINARYSWITCHMENU_H_
...
class BinarySwitchMenu: public CommandMenu
{
public:
    BinarySwitchMenu();
    virtual ~BinarySwitchMenu();
    /*
...
BinarySwitchMenu::BinarySwitchMenu() :
        m_quit(false), m_mutex()
{
    onFindResourceCb = std::bind(&BinarySwitchMenu::onFindResource, this, std::placeholders::_1);
    print();
}

BinarySwitchMenu::~BinarySwitchMenu() {}

void BinarySwitchMenu::init(std::set<std::shared_ptr<OC::OCResource>, OCResourceComp> binarySwitchSet) {
...
std::string BinarySwitchMenu::getName()
{
    return "Binary Switch";
}

void BinarySwitchMenu::print()
{
    std::cout << "------------------------------------" << std::endl;
    std::cout << getName() << std::endl;
    std::cout << "------------------------------------" << std::endl;
    std::cout << "find) Discover any binary switch " << std::endl;
    std::cout << "list) list currently discovered binary switch services (" << m_binarySwitchVector.size() << " found)" << std::endl;
...
void BinarySwitchMenu::help() {
    print();
}
void BinarySwitchMenu::run(const std::vector<std::string>& cmd, std::stack<std::unique_ptr<CommandMenu>>& menuStack)
{
    if(cmd.size() > 0) {
        if (cmd[0] == "find") {
            OCStackResult result = OC::OCPlatform::findResource("", std::string(OC_RSRVD_WELL_KNOWN_URI) +  "?rt=" + UPNP_OIC_TYPE_POWER_SWITCH, CT_DEFAULT, onFindResourceCb);
            std::cout << "findResource(" + std::string(OC_RSRVD_WELL_KNOWN_URI) +  "?rt=" + UPNP_OIC_TYPE_POWER_SWITCH + ") - " << result << std::endl;
        } else if (cmd[0] == "list") {
            ...
            if(cmd.size() > 1) {
                try {
                    ...
                    bswitch.isOnAsync([bswitch](const OC::HeaderOptions &headerOptions, const OC::OCRepresentation &rep, const int eCode)
                                {
                    ...
                    if ("all" == cmd[1]) {
                        ...
                        BinarySwitch b(r);
                        b.isOnAsync([b](const OC::HeaderOptions &headerOptions, const OC::OCRepresentation &rep, const int eCode)
                                {
                            if (eCode == OC_STACK_OK)
                            {
                                bool powerState;
            ...
            if(cmd.size() > 1) {
                try {
                    ...
                    BinarySwitch bswitch(m_binarySwitchVector[i]);
                    bswitch.turnOnAsync(true, [](const OC::HeaderOptions &headerOptions, const OC::OCRepresentation &rep, const int eCode){});
                } catch (const std::invalid_argument& ia) {
                    if ("all" == cmd[1]) {
                        for (auto r : m_binarySwitchVector) {
                            BinarySwitch b(r);
                            b.turnOnAsync(true, [](const OC::HeaderOptions &headerOptions, const OC::OCRepresentation &rep, const int eCode) {});
                        }
                    }
                }
            } else {
            ...
            if(cmd.size() > 1) {
                try {
                    ...
                    BinarySwitch bswitch(m_binarySwitchVector[i]);
                    bswitch.turnOnAsync(false, [](const OC::HeaderOptions &headerOptions, const OC::OCRepresentation &rep, const int eCode){});
                } catch (const std::invalid_argument& ia) {
                    if ("all" == cmd[1]) {
                        for (auto r : m_binarySwitchVector) {
                            BinarySwitch b(r);
                            b.turnOnAsync(false, [](const OC::HeaderOptions &headerOptions, const OC::OCRepresentation &rep, const int eCode) {});
                        }
                    }
                }
            } else {
                    ...
                    if ("all" == cmd[1]) {
                        ...
                        b.isOnAsync([b](const OC::HeaderOptions &headerOptions, const OC::OCRepresentation &rep, const int eCode)
                            ...
                            if (eCode == OC_STACK_OK)
                            {
                                ...
                                //std::cout << uri << std::endl;
                                //std::cout << "\t" << ((powerState) ? "is ON" : "is OFF") << std::endl;
                                b.turnOnAsync(!powerState, [](const OC::HeaderOptions &headerOptions, const OC::OCRepresentation &rep, const int eCode){});
                            }
                                });
                        }
                    }
...
bool BinarySwitchMenu::quit() {
    return m_quit;
}

void BinarySwitchMenu::onFindResource(std::shared_ptr< OC::OCResource > resource)
{
    std::lock_guard<std::mutex> lock(m_mutex);
