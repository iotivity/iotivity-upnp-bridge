...
void MenuBinarySwitch::init(std::set<std::shared_ptr<OC::OCResource>, OCResourceComp> binarySwitchSet) {
...
void MenuBinarySwitch::run(const std::vector<std::string>& cmd, std::stack<std::unique_ptr<MenuBase>>& menuStack)
{
    if(cmd.size() > 0) {
        if (cmd[0] == "find") {
