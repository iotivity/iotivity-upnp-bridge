        ...
        if (cmd[0] == "find") {
            ...
            if(cmd.size() > 1) {
                try {
                    ...
                    m_light_instance_vector[i].isOnAsync([](const OC::HeaderOptions &headerOptions, const OC::OCRepresentation &rep, const int eCode)
                            {
                    ...
                    if ("all" == cmd[1]) {
                        for (auto r : m_light_instance_vector) {
                            r.isOnAsync([](const OC::HeaderOptions &headerOptions, const OC::OCRepresentation &rep, const int eCode)
                                    {
                                if (eCode == OC_STACK_OK)
                                {
                                    bool powerState;
            ...
            if(cmd.size() > 1) {
                try {
                    ...
                    m_light_instance_vector[i].getBrightnessAsync([](const OC::HeaderOptions &headerOptions, const OC::OCRepresentation &rep, const int eCode)
                            {
                        if (eCode == OC_STACK_OK)
                        {
                            int brightness;
                    ...
                    if ("all" == cmd[1]) {
                        for (auto r : m_light_instance_vector) {
                            r.getBrightnessAsync([](const OC::HeaderOptions &headerOptions, const OC::OCRepresentation &rep, const int eCode)
                                    {
                                if (eCode == OC_STACK_OK)
                                {
                                    int brightness;
