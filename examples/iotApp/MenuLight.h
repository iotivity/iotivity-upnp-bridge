...
void MenuLight::print()
{
    ...
    std::cout << "list) list currently discovered lights (" << m_lightVector.size() << " found)" << std::endl;
    ...
    std::cout << "setb) setb <#> <0-100> or setb all - set the brightness %0-100  for the light" << std::endl;
...
void MenuLight::run(const std::vector<std::string>& cmd, std::stack<std::unique_ptr<MenuBase>>& menuStack)
{
    if(cmd.size() > 0) {
        if (cmd[0] == "find") {
            OCStackResult result = OC::OCPlatform::findResource("", std::string(OC_RSRVD_WELL_KNOWN_URI) +  "?rt=" + UPNP_OIC_TYPE_DEVICE_LIGHT, CT_DEFAULT, onFindResourceCb);
            std::cout << "findResource(" + std::string(OC_RSRVD_WELL_KNOWN_URI) +  "?rt=" + UPNP_OIC_TYPE_DEVICE_LIGHT + ") - " << result << std::endl;
        } else if (cmd[0] == "list") {
            ...
            if(cmd.size() > 1) {
                try {
                    ...
                    m_light_instance_vector[i].isOnAsync([/*bswitch*/](const OC::HeaderOptions &headerOptions, const OC::OCRepresentation &rep, const int eCode)
                            {
                    ...
                    if ("all" == cmd[1]) {
                        for (auto r : m_light_instance_vector) {
                            r.isOnAsync([/*b*/](const OC::HeaderOptions &headerOptions, const OC::OCRepresentation &rep, const int eCode)
                                    {
                                if (eCode == OC_STACK_OK)
                                {
                                    bool powerState;
            ...
            if(cmd.size() > 1) {
                try {
                    ...
                    m_light_instance_vector[i].turnOnAsync(true, [](const OC::HeaderOptions &headerOptions, const OC::OCRepresentation &rep, const int eCode){});
                } catch (const std::invalid_argument& ia) {
                    if ("all" == cmd[1]) {
                        for (auto r : m_light_instance_vector) {
                            r.turnOnAsync(true, [](const OC::HeaderOptions &headerOptions, const OC::OCRepresentation &rep, const int eCode) {});
                        }
                    }
                }
            } else {
            ...
            if(cmd.size() > 1) {
                try {
                    ...
                    m_light_instance_vector[i].turnOnAsync(false, [](const OC::HeaderOptions &headerOptions, const OC::OCRepresentation &rep, const int eCode){});
                } catch (const std::invalid_argument& ia) {
                    if ("all" == cmd[1]) {
                        for (auto r : m_light_instance_vector) {
                            r.turnOnAsync(false, [](const OC::HeaderOptions &headerOptions, const OC::OCRepresentation &rep, const int eCode) {});
                        }
                    }
                }
            } else {
            ...
            if(cmd.size() > 1) {
                try {
                    ...
                    l.isOnAsync([l](const OC::HeaderOptions &headerOptions, const OC::OCRepresentation &rep, const int eCode)
                            {
                        if (eCode == OC_STACK_OK)
                        {
                            bool powerState;
                            rep.getValue("value", powerState);
                            l.turnOnAsync(!powerState, [](const OC::HeaderOptions &headerOptions, const OC::OCRepresentation &rep, const int eCode){});
                        }
                            });
                    ...
                    if ("all" == cmd[1]) {
                        for (auto r : m_light_instance_vector) {
                            r.isOnAsync([r](const OC::HeaderOptions &headerOptions, const OC::OCRepresentation &rep, const int eCode)
                                    {
                                if (eCode == OC_STACK_OK)
                                {
                                    bool powerState;
                                    rep.getValue("value", powerState);
                                    r.turnOnAsync(!powerState, [](const OC::HeaderOptions &headerOptions, const OC::OCRepresentation &rep, const int eCode){});
                                }
                                    });
                        }
                    }
            ...
            if(cmd.size() > 1) {
                try {
                    ...
                    m_light_instance_vector[i].getBrightnessAsync([/*b*/](const OC::HeaderOptions &headerOptions, const OC::OCRepresentation &rep, const int eCode)
                            {
                        if (eCode == OC_STACK_OK)
                        {
                            int brightness;
                    ...
                    if ("all" == cmd[1]) {
                        for (auto r : m_light_instance_vector) {
                            r.getBrightnessAsync([/*b*/](const OC::HeaderOptions &headerOptions, const OC::OCRepresentation &rep, const int eCode)
                                    {
                                if (eCode == OC_STACK_OK)
                                {
                                    int brightness;
            ...
            if(cmd.size() > 2) {
                ...
                try {
                    ...
                    m_light_instance_vector[i].setBrightnessAsync(brightness, [](const OC::HeaderOptions &headerOptions, const OC::OCRepresentation &rep, const int eCode){});
                } catch (const std::invalid_argument& ia) {
                    if ("all" == cmd[1]) {
                        for (auto r : m_light_instance_vector) {
                            r.setBrightnessAsync(brightness, [](const OC::HeaderOptions &headerOptions, const OC::OCRepresentation &rep, const int eCode) {});
                        }
                    }
                }
            } else {
