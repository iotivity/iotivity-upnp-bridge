        ...
        m_resources(), m_quit(false)
{
    ...
    onResponceCb = std::bind(&MenuIotivity::onResponse, this, std::placeholders::_1, std::placeholders::_2, std::placeholders::_3);
    onObserveCb = std::bind(&MenuIotivity::onObserve, this, std::placeholders::_1, std::placeholders::_2, std::placeholders::_3, std::placeholders::_4);
    print();
}
...
void MenuIotivity::run(const std::vector<std::string>& cmd, std::stack<std::unique_ptr<MenuBase>>& menuStack)
{
    if(cmd.size() > 0) {
        if (cmd[0] == "find") {
            ...
            if(cmd.size() > 1) {
                try {
                    ...
                    std::cout << "get " << m_resources[i]->host() << m_resources[i]->uri() << " - " << result << std::endl;
                } catch (const std::invalid_argument& ia) {
            ...
            try {
                ...
                std::cout << "put " << m_resources[i]->host() << m_resources[i]->uri() << " - " << result << std::endl;
            } catch (const std::invalid_argument& ia) {
            ...
            try {
                ...
                while (k < cmd.size()) {
                    ...
                    if (type == "int") {
                        ...
                        std::cout << "Unsupported parameter type type must be 'int', 'double', 'bool' or 'string'." << std::endl;
                    }
                }
                OC::QueryParamsMap queryParams;
                OCStackResult result = m_resources[i]->post(rep, queryParams, onResponceCb);
                std::cout << "post " << m_resources[i]->host() << m_resources[i]->uri() << " - " << result << std::endl;
            } catch (const std::invalid_argument& ia) {
            ...
            try {
                ...
                std::cout << "observe " << m_resources[i]->host() << m_resources[i]->uri() << " - " << result << std::endl;
            } catch (const std::invalid_argument& ia) {
                std::cout << "The post command requires a second number parameter the number must\n"
                        "represent a found resource. try 'list' or 'find' command." << std::endl;
            }
...
void MenuIotivity::onResponse(const OC::HeaderOptions&, const OC::OCRepresentation& rep, const int eCode)
{
...
void MenuIotivity::onObserve(const OC::HeaderOptions&, const OC::OCRepresentation& rep, const int eCode, const int sequenceNumber)
{
