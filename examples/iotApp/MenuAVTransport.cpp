...
void MenuAVTransport::print()
{
    ...
    std::cout << "list) list currently discovered AV transport services (" << m_avTransportVector.size() << " found)" << std::endl;
    std::cout << "setavturi) setavturi <#> <instanceId> <currentURI> <currentURIMetaData> - set the  transport URI" << std::endl;
    std::cout << "setnextavturi) setnextavturi <#> <instanceId> <nextURI> <nextURIMetaData> - get the sort capabilities" << std::endl;
    ...
    std::cout << "seek) seek <#> <instanceId> - seek" << std::endl;
                               ...
                               std::stack<std::unique_ptr<MenuBase>> &menuStack)
{
    if (cmd.size() > 0)
    {
        ...
        else if ("seek" == cmd[0])
        {
            ...
            {
                std::cout << "seek command requires an integer, and instanceId, seekUnit, and seekTarget." << std::endl;
            }
        }
        else if ("next" == cmd[0])
        {
        ...
        else if ("getcta" == cmd[0])
        {
            if (cmd.size() > 1)
            {
                try
                {
                    ...
                    AVTransport avt(m_avTransportVector[i]);
                    std::cout << m_avTransportVector[i]->uri() << std::endl;
                    std::cout << "Current Transport Actions: " << avt.getCurrentTransportActions(instanceid) << std::endl;
                }
                catch (const std::invalid_argument &ia)
                {
                    std::cout << "unable to convert string to int." << std::endl;
