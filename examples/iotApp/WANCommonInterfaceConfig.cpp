...
void WANCommonInterfaceConfig::setEnabledForInternet(bool inetEnabled)
{
    ...
    m_setEnabledForInternetCB = bind(&WANCommonInterfaceConfig::onSetEnabledForInternet, this,
...
void WANCommonInterfaceConfig::onSetEnabledForInternet(const HeaderOptions &headerOptions,
...
bool WANCommonInterfaceConfig::getEnabledForInternet()
{
    unique_lock<mutex> wanLock(m_mutex);
    m_inetEnabled = false;
    m_getEnabledForInternetCB = bind(&WANCommonInterfaceConfig::onGetEnabledForInternet, this,
                                     placeholders::_1, placeholders::_2, placeholders::_3);
...
void WANCommonInterfaceConfig::onGetEnabledForInternet(const HeaderOptions &headerOptions,
        const OCRepresentation &rep, const int eCode)
{
    (void) headerOptions;
...
WANCommonInterfaceConfig::LinkProperties WANCommonInterfaceConfig::getCommonLinkProperties()
{
    unique_lock<mutex> wanLock(m_mutex);
    m_linkProperties = {"", 0, 0, ""};
    m_getCommonLinkPropertiesCB = bind(&WANCommonInterfaceConfig::onGetCommonLinkProperties, this,
                                       placeholders::_1, placeholders::_2, placeholders::_3);
...
void WANCommonInterfaceConfig::onGetCommonLinkProperties(const HeaderOptions &headerOptions,
        const OCRepresentation &rep, const int eCode)
{
    (void) headerOptions;
    if (eCode == OC_STACK_OK)
...
string WANCommonInterfaceConfig::getWANAccessProvider()
{
    unique_lock<mutex> wanLock(m_mutex);
    m_accessProvider = "";
    m_getWANAccessProviderCB = bind(&WANCommonInterfaceConfig::onGetWANAccessProvider, this,
                                    placeholders::_1, placeholders::_2, placeholders::_3);
...
void WANCommonInterfaceConfig::onGetWANAccessProvider(const HeaderOptions &headerOptions,
        const OCRepresentation &rep, const int eCode)
{
    (void) headerOptions;
    if (eCode == OC_STACK_OK)
...
int WANCommonInterfaceConfig::getMaximumActiveConnections()
{
    unique_lock<mutex> wanLock(m_mutex);
    m_maxActiveConnections = 0;
    m_getMaximumActiveConnectionsCB = bind(&WANCommonInterfaceConfig::onGetMaximumActiveConnections,
...
void WANCommonInterfaceConfig::onGetMaximumActiveConnections(const HeaderOptions &headerOptions,
        const OCRepresentation &rep, const int eCode)
{
    (void) headerOptions;
    if (eCode == OC_STACK_OK)
...
int WANCommonInterfaceConfig::getTotalBytesSent()
{
    unique_lock<mutex> wanLock(m_mutex);
    m_totalBytesSent = 0;
    m_getTotalBytesSentCB = bind(&WANCommonInterfaceConfig::onGetTotalBytesSent, this, placeholders::_1,
...
void WANCommonInterfaceConfig::onGetTotalBytesSent(const HeaderOptions &headerOptions,
        const OCRepresentation &rep, const int eCode)
{
    (void) headerOptions;
    if (eCode == OC_STACK_OK)
...
int WANCommonInterfaceConfig::getTotalBytesReceived()
{
    unique_lock<mutex> wanLock(m_mutex);
    m_totalBytesReceived = 0;
    m_getTotalBytesSentCB = bind(&WANCommonInterfaceConfig::onGetTotalBytesReceived, this,
                                 placeholders::_1, placeholders::_2, placeholders::_3);
    m_resource->get(QueryParamsMap(), m_getTotalBytesSentCB);
    if (m_cv.wait_for(wanLock, chrono::seconds(MAX_WAIT_TIME_FOR_BLOCKING_CALL)) == cv_status::timeout)
    {
...
void WANCommonInterfaceConfig::onGetTotalBytesReceived(const HeaderOptions &headerOptions,
        const OCRepresentation &rep, const int eCode)
{
    (void) headerOptions;
    if (eCode == OC_STACK_OK)
...
int WANCommonInterfaceConfig::getTotalPacketsSent()
{
    unique_lock<mutex> wanLock(m_mutex);
    m_totalPacketsSent = 0;
    m_getTotalPacketsSentCB = bind(&WANCommonInterfaceConfig::onGetTotalPacketsSent, this,
                                   placeholders::_1, placeholders::_2, placeholders::_3);
...
void WANCommonInterfaceConfig::onGetTotalPacketsSent(const HeaderOptions &headerOptions,
        const OCRepresentation &rep, const int eCode)
{
    (void) headerOptions;
    if (eCode == OC_STACK_OK)
...
int WANCommonInterfaceConfig::getTotalPacketsReceived()
{
    unique_lock<mutex> wanLock(m_mutex);
    m_totalPacketsReceived = 0;
    m_getTotalPacketsReceivedCB = bind(&WANCommonInterfaceConfig::onGetTotalPacketsReceived, this,
                                       placeholders::_1, placeholders::_2, placeholders::_3);
...
void WANCommonInterfaceConfig::onGetTotalPacketsReceived(const HeaderOptions &headerOptions,
        const OCRepresentation &rep, const int eCode)
{
    (void) headerOptions;
    if (eCode == OC_STACK_OK)
...
int WANCommonInterfaceConfig::getNumberOfActiveConnections()
{
    unique_lock<mutex> wanLock(m_mutex);
    m_numberOfConnections = 0;
    m_getNumberOfActiveConnectionsCB = bind(&WANCommonInterfaceConfig::onGetNumberOfActiveConnections,
                                            this, placeholders::_1, placeholders::_2, placeholders::_3);
...
void WANCommonInterfaceConfig::onGetNumberOfActiveConnections(const HeaderOptions &headerOptions,
        const OCRepresentation &rep, const int eCode)
{
    (void) headerOptions;
    if (eCode == OC_STACK_OK)
...
WANCommonInterfaceConfig::ConnectionInfo WANCommonInterfaceConfig::getActiveConnection()
{
    unique_lock<mutex> wanLock(m_mutex);
    m_activeConnection = {"", ""};
    m_getActiveConnectionCB = bind(&WANCommonInterfaceConfig::onGetActiveConnection, this,
                                   placeholders::_1, placeholders::_2, placeholders::_3);
...
void WANCommonInterfaceConfig::onGetActiveConnection(const HeaderOptions &headerOptions,
        const OCRepresentation &rep, const int eCode)
{
    (void) headerOptions;
    if (eCode == OC_STACK_OK)
