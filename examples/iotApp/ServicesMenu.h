...
#ifndef SERVICESMENU_H_
#define SERVICESMENU_H_
...
#include "BinarySwitchMenu.h"

class ServicesMenu: public CommandMenu
{
public:
    ServicesMenu();
    virtual ~ServicesMenu();
...
ServicesMenu::ServicesMenu():
        m_quit(false)
{
    onFindResourceCb = std::bind(&ServicesMenu::onFindResource, this, std::placeholders::_1);
    print();
}

ServicesMenu::~ServicesMenu() {}

std::string ServicesMenu::getName()
{
    return "Services";
}

void ServicesMenu::print()
{
    // Services that are unimplemented are currently commented out these can quickly be added as
    // the menu is created.
    std::cout << "------------------------------------" << std::endl;
    ...
    //std::cout << "av) " << UPNP_OIC_TYPE_AV_TRANSPORT << " (" << m_avTransport.size() << " found)" << std::endl;
    //std::cout << "brightness) " << UPNP_OIC_TYPE_BRIGHTNESS << " (" << m_brightness.size() << " found)" << std::endl;
    std::cout << "switch) " << UPNP_OIC_TYPE_POWER_SWITCH << " (" << m_binarySwitch.size() << " found)" << std::endl;
    //std::cout << "connection) " << UPNP_OIC_TYPE_CONNECTION_MANAGER << " (" << m_connectionManager.size() << " found)" << std::endl;
    //std::cout << "content) " << UPNP_OIC_TYPE_CONTENT_DIRECTORY << " (" << m_contentDirectory.size() << " found)" << std::endl;
    //std::cout << "layer3) " << UPNP_OIC_TYPE_LAYER3_FORWARDING << " (" << m_layer3Forwarding.size() << " found)" << std::endl;
    //std::cout << "rendering) " << UPNP_OIC_TYPE_RENDERING_CONTROL << " (" << m_renderingControl.size() << " found)" << std::endl;
    //std::cout << "recording) " << UPNP_OIC_TYPE_SCHEDULED_RECORDING << " (" << m_renderingControl.size() << " found)" << std::endl;
    //std::cout << "ifconfig) " << UPNP_OIC_TYPE_WAN_IF_CONFIG << " (" << m_wanIfConfig.size() << " found)" << std::endl;
...
void ServicesMenu::help() {
    print();
}
void ServicesMenu::run(const std::vector<std::string>& cmd, std::stack<std::unique_ptr<CommandMenu>>& menuStack)
{
    if(cmd.size() > 0) {
        if (cmd[0] == "find") {
            ...
            std::unique_ptr<BinarySwitchMenu> menuItem(new BinarySwitchMenu);
...
bool ServicesMenu::quit() {
    return m_quit;
}

void ServicesMenu::onFindResource(std::shared_ptr< OC::OCResource > resource)
{
