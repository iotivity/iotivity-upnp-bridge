...
void MenuRenderingControl::print()
{
    ...
    std::cout << "list) list currently discovered content directory services (" <<
              m_renderingControlVector.size() << " found)" << std::endl;
    //TODO add last change
    std::cout << "listpresets) listpresets <#> <instanceId> - obtain the preset name list" << std::endl;
    std::cout << "selectpreset) selectpreset <#> <instanceId> <presetName> - select preset" <<
              std::endl;
    std::cout <<
    ...
    std::cout <<
              ...
              << std::endl;
    std::cout <<
              "setvol) setvol <#> <instanceId> [channel] <desiredMute> - set volume (Master channel is used if channel is unspecified)"
              << std::endl;
