...
void MenuDevices::print()
{
    ...
    std::cout << "inetgateway) " << UPNP_OIC_TYPE_DEVICE_INET_GATEWAY << " (" << m_inetGateway.size() << " found)" << std::endl;
    std::cout << "lan) " << UPNP_OIC_TYPE_DEVICE_LAN << " (" << m_lan.size() << " found)" << std::endl;
    std::cout << "light) " << UPNP_OIC_TYPE_DEVICE_LIGHT << " (" << m_light.size() << " found)" << std::endl;
    std::cout << "mrenderer) " << UPNP_OIC_TYPE_DEVICE_MEDIA_RENDERER << " (" << m_mediaRenderer.size() << " found)" << std::endl;
    std::cout << "mserver) " << UPNP_OIC_TYPE_DEVICE_MEDIA_SERVER << " (" << m_mediaServer.size() << " found)" << std::endl;
    std::cout << "wan) " << UPNP_OIC_TYPE_DEVICE_WAN << " (" << m_wan.size() << " found)" << std::endl;
    std::cout << "wanconn) " << UPNP_OIC_TYPE_DEVICE_WAN_CONNECTION << " (" << m_wanConnection.size() << " found)" << std::endl;
...
void MenuDevices::run(const std::vector<std::string>& cmd, std::stack<std::unique_ptr<MenuBase>>& menuStack)
{
    if(cmd.size() > 0) {
        if (cmd[0] == "find") {
