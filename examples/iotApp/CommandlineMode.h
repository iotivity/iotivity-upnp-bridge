    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (0 == strcmp("--help", token) || 0 == strcmp("-h", token)) {
            ...
            try {
                ...
                std::cerr << "Invalid input for --time option" << std::endl;
                printHelp();
                exit(0);
            }
            if(waitTime < 0) {
                std::cerr << "--time option MUST be a positive value" << std::endl;
                printHelp();
                exit(0);
            }
            ...
            if (nullptr != token) {
                if (token[0] != '-') {
                    ...
                    std::cerr << "--delay option MUST be a positive value" << std::endl;
                    printHelp();
                    exit(0);
                }
            } else {
