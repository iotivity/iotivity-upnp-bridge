                                        ...
                                        if(l.getValue("rt", rt)) {
                                            ...
                                            if("oic.r.switch.binary" == rt) {
                                                if(l.hasAttribute("href")) {
                                                    ...
                                                    OCPlatform::findResource("", string(OC_RSRVD_WELL_KNOWN_URI), CT_DEFAULT, bind(&Light::foundServices, this, placeholders::_1));
                                                }
            ...
            for (auto &resourceType : resource->getResourceTypes())
            {
                if (resourceType == UPNP_OIC_TYPE_DEVICE_LIGHT)
                {
                    if (s_lightLookup.find(resource->uri()) == s_lightLookup.end())
                    {
                        ...
                        light.sendGetRequest(bind(&Light::onGetServices, &s_lightLookup[resource->uri()], placeholders::_1, placeholders::_2, placeholders::_3));
                    }
                }
...
void listResources() {
    ...
    cout << "\t" << s_gatewayDeviceLookup.size() << " Internet Gateway Devices" << endl; //TODO add gateway device
    cout << "\t" << s_mediaRendererDeviceLookup.size() << " Media Renderer Devices" << endl; //TODO add gateway device
    cout << "\t" << s_mediaServerDeviceLookup.size() << " Media Server Devices" << endl; //TODO add gateway device
    cout << "\n\tTotal resources " << s_lightLookup.size() + s_gatewayDeviceLookup.size() + s_mediaRendererDeviceLookup.size() + s_mediaServerDeviceLookup.size() << endl;
    cout << "-----------------------------" << endl;
}
...
void onOffLight(string uri, bool state) {
    ...
    if(lightItr != s_lightLookup.end()) {
        ...
        cout << "Sending POST request to: " << lightItr->second.m_powerSwitch.m_uri << " value: " << ((state)? "ON" : "OFF") << endl;
        ...
        for(auto &l: s_lightLookup) {
            if(l.second.m_powerSwitch.m_uri == uri) {
                found = true;
                std::cout << "Sending POST request to: " << l.second.m_powerSwitch.m_uri << " value: " << ((state)? "ON" : "OFF") << endl;
                OCRepresentation rep;
...
void onGetToggleLight(const HeaderOptions &headerOptions, const OCRepresentation &rep, const int eCode)
{
    (void) headerOptions;
    try
    {
...
void dimLight(string uri, int value) {
    auto lightItr = s_lightLookup.find(uri);
    if(lightItr != s_lightLookup.end()) {
        std::cout << "Sending POST request to: " << lightItr->second.m_brightness.m_uri << "loadlevel: " << value << "%" << std::endl;
        OCRepresentation rep;
